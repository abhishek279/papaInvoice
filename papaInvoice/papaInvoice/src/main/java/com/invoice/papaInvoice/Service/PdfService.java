...
public class PdfService {
    ...
    public ByteArrayInputStream generateQuotationPdf(Quotation quotation) throws Exception {
        ...
        Paragraph footer = new Paragraph("Terms & Conditions:\n")
                .add("1.Payment 30 Days\n")
                .add("2.Transportation Extra\n")
                .add("3.Delivery 3-4 Days after receiving PO\n")
                .add("4.GST Extra")
    ...
    public ByteArrayInputStream generateInvoicePdf(Invoice invoice) throws MalformedURLException {
        ...
        Color headerBgColor = new DeviceRgb(63, 169, 219); // Light blue background
        table.addHeaderCell(createStyledCell("S.No", headerBgColor, true,fontSize, padding));
        table.addHeaderCell(createStyledCell("Product Description", headerBgColor, true,fontSize, padding));
        ...
        NumberFormat currencyFormat = NumberFormat.getCurrencyInstance(locale);
        int sNo = 1;
        for (ProductItem item : invoice.getProductItems()) {
            // Add a single row for demonstration. Loop through your items instead.
            table.addCell(createStyledCell(String.valueOf(sNo++), ColorConstants.WHITE, false, fontSize, padding));
            table.addCell(createStyledCell(item.getDescription(), ColorConstants.WHITE, false, fontSize, padding));
            table.addCell(createStyledCell(String.valueOf(item.getQuantity()), ColorConstants.WHITE, false, fontSize, padding));
            table.addCell(createStyledCell(currencyFormat.format(item.getRate()), ColorConstants.WHITE, false, fontSize, padding));
            table.addCell(createStyledCell(currencyFormat.format(item.getGstRate()), ColorConstants.WHITE, false, fontSize, padding));
        ...
        String formattedGrandTotal = currencyFormat.format(invoice.getGrandTotal());

// Create a paragraph to display the grand total
        Paragraph grandTotalParagraph = new Paragraph("Grand Total: " + formattedGrandTotal)
                .setBold()
                .setTextAlignment(TextAlignment.RIGHT)
                .setFontSize(12);
        document.add(new Paragraph("\n"));
...
// Add the grand total paragraph to the document
        document.add(new Paragraph("\n"));
